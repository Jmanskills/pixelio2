...
router.post('/announce', adminAuth, async (req, res) => {
  try {
    ...
    const io = getIO();
    if (!io) return res.status(500).json({ error: 'Server not ready.' });
    io.emit('announcement', { message: message.trim(), from: req.admin.username });
    res.json({ message: `Announcement sent to all players.` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/tempban', adminAuth, async (req, res) => {
  try {
    const { username, hours, reason } = req.body;
    if (!username || !hours || Number(hours) < 1) return res.status(400).json({ error: 'Username and hours required.' });
    const user = await User.findOne({ username });
    if (!user) return res.status(404).json({ error: 'User not found.' });
    if (isOwner(username)) return res.status(400).json({ error: 'Cannot ban the owner.' });
    const unbanAt = new Date(Date.now() + Number(hours) * 3600000);
    user.isBanned = true;
    user.banReason = reason || `Temp ban for ${hours}h`;
    user.unbanAt = unbanAt;
    await user.save();
    // Kick them if online
    try {
      const { getOnlineUsers, getIO } = require('../game');
      const socketId = getOnlineUsers()[username];
      if (socketId) getIO().to(socketId).emit('kicked', { reason: `You have been temporarily banned for ${hours} hour(s).` });
    } catch {}
    res.json({ message: `${username} temp-banned for ${hours}h (until ${unbanAt.toLocaleString()}).` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/mute', adminAuth, async (req, res) => {
  try {
    const { username, minutes } = req.body;
    if (!username || !minutes || Number(minutes) < 1) return res.status(400).json({ error: 'Username and minutes required.' });
    const { getOnlineUsers, getIO } = require('../game');
    const io = getIO(); const onlineUsers = getOnlineUsers();
    const socketId = onlineUsers[username];
    if (socketId) io.to(socketId).emit('muted', { minutes: Number(minutes) });
    res.json({ message: `${username} muted for ${minutes} minutes.` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post('/unmute', adminAuth, async (req, res) => {
  try {
    ...
    const { getOnlineUsers, getIO } = require('../game');
    const socketId = getOnlineUsers()[username];
    if (socketId) getIO().to(socketId).emit('unmuted');
    res.json({ message: `${username} unmuted.` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/matches', adminAuth, async (req, res) => {
  try {
    ...
    const active = Object.values(rooms)
      .filter(r => r.started && !r.winner)
      .map(r => ({
        roomId: r.roomId,
        isPractice: !!r.isPractice,
        players: Object.values(r.players).map(p => ({
          username: p.username, hp: p.hp, alive: p.alive
        }))
      }));
    res.json({ matches: active, count: active.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
